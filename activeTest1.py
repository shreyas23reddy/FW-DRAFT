...
if __name__=='__main__':
    ...
    while True:
        ...
        print(f"Active Policy - ID: {vSmartpolicyId} - Name {vSmartActivePolicy['policyName']} ")

        config.update({"oldActivevSmartpolicyId":vSmartpolicyId})
        with open("vmanage_login1.yaml",'w') as yamlfile:
                yaml.safe_dump(config, yamlfile)


        print(f"Collecting the policy attached to Site's")
        sitePolicyActive = getData.getPolicySiteListID(vmanage_host,vmanage_port,header)


        print(f"parsing the collected data")
        ...
        for activeDataPolicysiteLisdID,index in zip( infoCollectedDictSite['activeDataPolicySiteID'], range(len(infoCollectedDictSite['activeDataPolicySiteID'])) ):
            ...
            if len(activeDataPolicysiteLisdID) > 0 and activeDataPolicysiteLisdID != []:
                ...
                if infoCollectedDictSite["siteRange"][index][0] < infoCollectedDictSite["siteRange"][index][1]:
                    ...
                    print(f"created Site {site_ID} Exclusion List {SiteListExSiteID}")
                    print(f"created Site {site_ID} only List {SiteListInSiteID}")

                    config.update({"SiteListSiteID":[SiteListExSiteID,SiteListInSiteID]})
                    with open("vmanage_login1.yaml",'w') as yamlfile:
                            yaml.safe_dump(config, yamlfile)
                    ...
                    config.update({"SiteListSiteID":[SiteListInSiteID]})
                    with open("vmanage_login1.yaml",'w') as yamlfile:
                            yaml.safe_dump(config, yamlfile)
        ...
        print(f'Creted a new centralized Policy with the traffic redirection to Firewall services on {site_ID} {newPolicyID["policyId"]}')

        config.update({"newActivevSmartpolicyId":newPolicyID["policyId"]})
        with open("vmanage_login1.yaml",'w') as yamlfile:
                yaml.safe_dump(config, yamlfile)
        ...
        print("new policy was activated waiting for the push to be completed")
        ...
        while  vSmartPolicyActineNow != newPolicyID["policyId"]:
            time.sleep(10)
            vSmartPolicy = getData.getVsmartPolicy(vmanage_host,vmanage_port,header)
            for vSmartActivePolicy in vSmartPolicy:
                if vSmartActivePolicy["isPolicyActivated"] == True:
                    vSmartPolicyActineNow = vSmartActivePolicy['policyId']
                    break
        print(f" Push completed ID - {vSmartPolicyActineNow} {vSmartActivePolicy['policyName']} is active now")
